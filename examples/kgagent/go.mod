// kgagent is a module of its own for the reason connectors is: so that an
// agent framework lands neither in the core module, whose dependency list
// DESIGN.md §9 states and internal/claims checks on every run, nor in
// connectors, which a buyer imports in order to write a graph into their own
// store and should not have to compile a language model client for.
module github.com/liliang-cn/alchemy/examples/kgagent
...
require (
	...
	github.com/liliang-cn/cortexdb/v2 v2.90.0 // indirect
