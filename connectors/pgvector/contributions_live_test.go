...
func contributing(t *testing.T) (*Loader, string) {
	...
	if _, err := l.Load(context.Background(), contributed(), LoadOptions{ID: "contrib"}); err != nil {
		t.Fatalf("Load: %v", err)
	}
	return l, "contrib"
}
...
// An edge naming an entity this load does not describe is still a source that
// had a hand in that node, and this is the one store of the three that can say
// so.
//
// There is no foreign key from a relation to an entity here, deliberately: §7.3
// delivers ViolationDanglingRelation with the graph rather than refusing the
// load, so the row exists and can be read. Claims already renders such an edge —
// "a claim about something this load does not describe, which is different from
// no claim" — and this answers in the same terms, with the sources that referred
// to the node and an empty Type, rather than the zero Contributions that would
// say nobody mentioned it at all. The graph stores drop those edges when they
// write, so neither of them can reach this case; that is a difference between
// the stores and not a difference of opinion about the method.
func TestASourceThatNamedANodeThisLoadDoesNotDescribeIsStillReported(t *testing.T) {
	l := newFixture(t).open(t, Config{})
	ctx := context.Background()
	if _, err := l.Load(ctx, pack("SuperAI uses CortexDB.", 100, 122), LoadOptions{ID: "dangling"}); err != nil {
		t.Fatalf("Load: %v", err)
	}
	got, err := l.Contributions(ctx, "dangling", "e-not-here")
	if err != nil {
		t.Fatalf("Contributions: %v", err)
	}
	...
	if !reflect.DeepEqual(got, want) {
		t.Errorf("Contributions(e-not-here) =\n%+v\nwant\n%+v", got, want)
	}
}
