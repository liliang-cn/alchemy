...
//
// This is the failure as a buyer met it: Ravel's own schema, uploaded as a
// DDL source, came back held at NEEDS_REVIEW on conflicts that were all false
// — five of its tables reference one table twice, which is how you model a
// relationship between two rows of one table, and the verifier read each pair
// as two sources disagreeing about one edge. §7.3 will not let a caller opt out
// of a person, so the job could never finish, and nothing a person could decide
// would have made it right.
//
// The file lives in pkg/verify/testdata, where the unit-level acceptance test
// for the same defect reads it, and is read from there rather than copied: it
// is GPL'd third-party code and one copy in the tree is the honest number.
func TestTheRavelSchemaRunsToTheEnd(t *testing.T) {
